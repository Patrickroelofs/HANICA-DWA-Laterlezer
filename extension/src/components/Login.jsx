...
function Login({ setUser }) {
  ...
  return (
    <div className="login p-8 text-center">
      ...
      {
        response
          ? <span className="text-red-600 font-bold">Something went wrong</span>
          : null
      }
      ...
      <button type="submit" onClick={() => usernameLogin(name)} className="bg-indigo-600 hover:bg-blue-dark text-white font-bold py-3 px-6 rounded-lg mt-3 ml-4 hover:bg-indigo-500 transition ease-in-out duration-300">Login</button>
    </div>
  );
}
